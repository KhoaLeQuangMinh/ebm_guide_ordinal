...
pickle_folder = "/Users/khoale/Downloads/Alzheimer_Code/sustain_output/pickle_files"

# Load the prepared dataset to match labels
df = pd.read_csv(data_path)

# List of regions (biomarkers) in order
regions = ["Frontal", "Temporal", "Parietal", "Occipital", "Cingulate", "Insula", 
           "Hippocampus", "Amygdala", "Caudate", "Pallidum", "Putamen", "Accumbens"]
N_bio = len(regions)

# Re-create the index mapping used by ZscoreSustain internally
Z_vals = np.array([[1, 2, 3]] * N_bio)
...
def analyze_pickle(pickle_name, num_subtypes):
    ...
    print("\n" + "="*80)
    print(f" ANALYZING MODEL: {num_subtypes} SUBTYPE(S) ".center(80, "="))
    print("="*80)
...
# Analyze both the 2-subtype and 3-subtype models
analyze_pickle("ADNI_MRI_subtype1.pickle", num_subtypes=2)
analyze_pickle("ADNI_MRI_subtype2.pickle", num_subtypes=3)