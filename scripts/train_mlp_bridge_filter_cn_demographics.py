"""
train_mlp_bridge_filter_cn_demographics.py
===========================================
Method 1 (Solution A - Filter CN + Demographics): 
6-Feature MLP Bridge mapping Filter CN SuStaIn subtype & stage predictions 
plus Subject Age and Sex to 4-class diagnosis (CN, sMCI, pMCI, AD).
"""
...
# ── PyTorch MLP Architecture (6-Input Features) ───────────────────────────────
...
def main():
    predictions_csv = '/Users/khoale/Downloads/Alzheimer_Code/sustain_filter_cn_predictions.csv'
    reference_csv   = '/Users/khoale/Downloads/Alzheimer_Code/sustain_subject_staging_results_filter_cn.csv'
    meta_csv        = '/Users/khoale/Downloads/Alzheimer_Code/TABLE_DATA_ADNI/MRI_T1_3D_ADNI1_2_BL_PRE_4_24_2026.csv'
    ...
    print(f"Loaded {len(df)} subject predictions from filter_cn predictions CSV.")

    # 2. Merge clinical diagnosis label from reference staging CSV
    if 'Label' not in df.columns and os.path.exists(reference_csv):
        ...
        print("Merged clinical diagnosis labels ('Label') from reference filter_cn staging CSV.")

    # 3. Merge Demographics (Age and Sex) from ADNI Metadata table
    if os.path.exists(meta_csv):
        meta_df = pd.read_csv(meta_csv)[['Subject', 'Sex', 'Age']].drop_duplicates(subset=['Subject'])
        df = df.merge(meta_df, left_on='PTID', right_on='Subject', how='left')
        print(f"Merged Age and Sex metadata for {df['Age'].notna().sum()}/{len(df)} subjects.")
    else:
        print(f"Error: Metadata CSV not found at {meta_csv}")
        return

    # Encode Sex to numeric binary (Male: 1.0, Female: 0.0)
    sex_mapping = {'M': 1.0, 'Male': 1.0, 'F': 0.0, 'Female': 0.0}
    df['Sex_Code'] = df['Sex'].map(sex_mapping).astype(float)
    ...
    df = df.dropna(subset=['Label_Code', 'Test_Fold', 'Age', 'Sex_Code'])
    df['Label_Code'] = df['Label_Code'].astype(int)

    unique_folds = sorted(df['Test_Fold'].unique())
    print(f"Starting 5-Fold Class-Weighted MLP Training (6 Features: Filter CN SuStaIn + Age + Sex) on folds: {unique_folds}\n")
    ...
    # 4. Perform Fold-by-Fold Training & Evaluation
    for fold_name in unique_folds:
        train_df = df[df['Test_Fold'] != fold_name].copy()
        test_df  = df[df['Test_Fold'] == fold_name].copy()

        # Input Features (6 numbers: SuStaIn 4-dim + Age + Sex)
        # Train on GROUND TRUTH SuStaIn features + Age + Sex
        X_train_raw = train_df[[
            'Prob_Subtype_1_True', 'Prob_Subtype_2_True', 'Prob_Subtype_3_True', 
            'Assigned_Stage_True', 'Age', 'Sex_Code'
        ]].values
        y_train = train_df['Label_Code'].values

        # Test on CNN PREDICTED SuStaIn features + Age + Sex
        X_test_raw = test_df[[
            'Prob_Subtype_1_Pred', 'Prob_Subtype_2_Pred', 'Prob_Subtype_3_Pred', 
            'Assigned_Stage_Pred', 'Age', 'Sex_Code'
        ]].values
        ...
        model = ClassWeightedMLP(input_dim=6, num_classes=4)
        ...
        print(f"--- {fold_name.upper()} (Filter CN Class-Weighted + Age & Sex) ---")
    ...
    # 5. Compile Out-Of-Fold (OOF) Overall Summary
    ...
    print("\n" + "="*70)
    print(" FILTER_CN CLASS-WEIGHTED OOF SUMMARY (WITH AGE & SEX) ".center(70, "="))
    print("="*70)
    ...
    export_path = os.path.join(output_dir, "mlp_bridge_filter_cn_demographics_4class_predictions.csv")
    ...
    print(f"\n✓ Exported filter_cn demographics predicted labels CSV to: {export_path}")
