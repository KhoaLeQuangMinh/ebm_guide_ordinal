"""
train_mlp_bridge_filter_cn.py
==============================
Method 1 (Solution A - Filter CN Variant): 
2-Layer MLP Bridge mapping SuStaIn subtype & stage predictions to 4-class 
diagnosis (CN, sMCI, pMCI, AD) trained specifically on the filter_cn SuStaIn model.
"""
...
# ── PyTorch MLP Architecture (4-Input Features) ───────────────────────────────
...
def main():
    ...
    print(f"Starting 5-Fold Class-Weighted MLP Training on filter_cn predictions across folds: {unique_folds}\n")
    ...
    for fold_name in unique_folds:
        ...
        # Input Features (4 numbers: Subtype 1, 2, 3 Probs + Assigned Stage)
        ...
        print(f"--- {fold_name.upper()} (Filter CN Class-Weighted MLP) ---")
    ...
    print("\n" + "="*70)
    print(" FILTER_CN CLASS-WEIGHTED OOF 4-CLASS DIAGNOSTIC SUMMARY ".center(70, "="))
    print("="*70)
    ...
    export_path = os.path.join(output_dir, "mlp_bridge_filter_cn_4class_predictions.csv")
    ...
    print(f"\n✓ Exported filter_cn class-weighted predicted labels CSV to: {export_path}")
